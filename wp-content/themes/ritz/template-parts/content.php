...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="detail-wrap">
		<header class="entry-header">
			...
			$cat_meta = ritz_get_option( 'category_meta' );
			...
			if ( ( 'post' === get_post_type() ) ) {
				...
				$author_meta 	= ritz_get_option( 'post_author' );
				
				$date_meta 		= ritz_get_option( 'post_date' ); 
				...
				if ( ( 1 !== absint( $author_meta ) ) && ( 1 !== absint( $date_meta ) ) ) { ?>
					...
					<span class="separator"><?php echo esc_html__( ' / ', 'ritz' ); ?></span>
			...
			<?php
				...
				if( is_single() ){
					...
					the_content( sprintf(
						...
						wp_kses( __( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'ritz' ), array( 'span' => array( 'class' => array() ) ) ),
						the_title( '<span class="screen-reader-text">"', '"</span>', false )
					) );

					wp_link_pages( array(
						'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'ritz' ),
						'after'  => '</div>',
					) );

					do_action( 'ritz_related_post' );
