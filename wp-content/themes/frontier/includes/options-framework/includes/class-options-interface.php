	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			case 'textarea':
				...
				$val = stripslashes( $val );
				$output .= '<textarea id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" rows="' . $rows . '" wrap="off">' . esc_textarea( $val ) . '</textarea>';
				break;
			...
			case "heading":
				...
				$output .= '<input type="submit" class="button-primary button-primary-top" name="update" value="' . __( 'Save Options', 'frontier' ) . '" />' . "\n";
				break;
			}
